...
class libro extends Controller
{
    ...
    */
    ...
    public function pdf($param = [])
    {
        ...
        // Comprobar si el usuario tiene permisos
        $this->checkPermissions($GLOBALS['libros']['pdf']);
        ...
        // Portada
        $pdf->Image("imagenes/portada.jpeg", 60, 23, 80, 30, "JPG");


        // Cuerpo listado
        $pdf->SetFont('Courier', '', 8);

        $pdf->SetFillColor(400, 330, 200); // Un color amarillo claro
        ...
        // Creo la propiedad libros para usar en la vista
        $librosQuery = $this->model->get();

        while ($libro = $librosQuery->fetch()) {
            $libros[] = [
                $libro->id,
                $libro->titulo,
                $libro->autor,
        ...
        foreach ($libros as $libro) {
            ...
            $pdf->Cell(10, 8, mb_convert_encoding($libro[0], 'ISO-8859-1', 'UTF-8'), 1, 0, 'C', $fondo);
            $pdf->Cell(45, 8, mb_convert_encoding($libro[1], 'ISO-8859-1', 'UTF-8'), 1, 0, 'L', $fondo);
            $pdf->Cell(35, 8, mb_convert_encoding($libro[2], 'ISO-8859-1', 'UTF-8'), 1, 0, 'L', $fondo);
            $pdf->Cell(30, 8, mb_convert_encoding($libro[3], 'ISO-8859-1', 'UTF-8'), 1, 0, 'L', $fondo);
            $pdf->Cell(40, 8, mb_convert_encoding($libro[4], 'ISO-8859-1', 'UTF-8'), 1, 0, 'L', $fondo);
            $pdf->Cell(10, 8, mb_convert_encoding($libro[5], 'ISO-8859-1', 'UTF-8'), 1, 0, 'R', $fondo);
            $pdf->Cell(20, 8, mb_convert_encoding($libro[6], 'ISO-8859-1', 'UTF-8'), 1, 1, 'L', $fondo);

            $fondo = !$fondo;   // Alternar fondo cada fila

        }

        // Hacer una mina de datos para ver la 2º página de abajo
