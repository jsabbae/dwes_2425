                                    ...
                                    <td>
                                        <div class="btn-group btn-group-toggle" data-toggle="buttons">
                                            <a href="<?= URL ?>libro/eliminar/<?= $libro->id ?>/<?= $_SESSION['csrf_token'] ?>" title="Eliminar"
                                                ...
                                                <?= !in_array($_SESSION['role_id'], $GLOBALS['libros']['eliminar'])? 'disabled':null ?>"
                                                onclick="return confirm('Confimar elimación del libro')" aria-label="Eliminar libro"><i
                                                    class="bi bi-trash-fill"></i></a>
                                            <a href="<?= URL ?>libro/editar/<?= $libro->id ?>/<?= $_SESSION['csrf_token'] ?>" title="Editar"
                                                class="btn btn-primary
                                                <?= !in_array($_SESSION['role_id'], $GLOBALS['libros']['editar'])? 'disabled':null ?>">
                                                <i class="bi bi-pencil-square"></i></a>
                                            <a href="<?= URL ?>libro/mostrar/<?= $libro->id ?>/<?= $_SESSION['csrf_token'] ?>" title="Mostrar"
                                                class="btn btn-warning 
                                                <?= !in_array($_SESSION['role_id'], $GLOBALS['libros']['mostrar'])? 'disabled':null ?>">
